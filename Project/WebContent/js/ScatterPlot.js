...
function BarChart(container) {
    ...
    model.when(["data", "xAttribute", "width"], function (data, xAttribute,
        ...
        svg.call(d3.behavior.zoom().scaleExtent([1, 1]).on(
            "zoom",
            function () {
        ...
        model.xScale = d3.scale.ordinal().rangeRoundBands([0, xWidth], 0.2, 0).domain(
...
function ScatterPlot(div) {
    var x = d3.scale.linear(), y = d3.scale.linear(), xAxis = d3.svg.axis()
        ...
        brushG = g.append('g')
            .attr('class', 'brush'),
        dotsG = g.append('g').attr('class', 'plots'),
        brush = d3.svg.brush().on('brush', brushed).on('brushend', brushend),
        dots, quadtree, model = Model();
